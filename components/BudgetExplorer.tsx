...
const getDefaultZoom = (level: NodeLevel) => (level === 'client' ? CLIENT_ZOOM : DEFAULT_ZOOM)
...
function buildClientHierarchy(rows: BudgetRow[]): ExplorerNode {
    ...
    .map(([category, categoryRows]) => {
      ...
      const clients = Array.from(groupedByClient.entries())
        .map(([company, companyRows]) => {
          const companyAmount = companyRows.reduce((sum, row) => sum + row.amount, 0)
  ...
  return {
    ...
    level: 'root',
    omnicomAmount: categories.reduce((sum, category) => sum + (category.omnicomAmount ?? 0), 0),
    children: categories,
  }
}
...
function lineUpCircles(nodes: ExplorerNode[], containerRadius: number): PackedCircle[] {
  ...
  let cursor =
    -(circles.reduce((sum, circle) => sum + circle.r * scale * 2, 0) + gap * (circles.length - 1)) /
    2
...
export default function BudgetExplorer() {
  ...
  const handleWheel = (event: React.WheelEvent<HTMLDivElement>) => {
    ...
    setZoom((value) =>
      Math.max(0.8, Math.min(2.5, value * (event.deltaY > 0 !== reverse ? 0.92 : 1.08))),
    )
  }
    ...
    <>
      ...
      <div className="relative hidden md:block">
        ...
        <input
          ...
          className="border-input bg-muted text-foreground placeholder:text-muted-foreground focus:border-ring focus:bg-muted w-64 rounded-xl border py-2 pr-3 pl-9 text-sm transition outline-none"
        />
        {searchResults.length > 0 ? (
          <div className="border-input bg-card absolute top-full right-0 z-30 mt-2 w-64 overflow-hidden rounded-2xl border shadow-2xl">
            {searchResults.map((result) => (
              <button
                ...
                className="border-border text-foreground hover:bg-muted hover:text-foreground flex w-full items-center justify-between border-b px-4 py-3 text-left text-sm transition last:border-b-0"
              >
    ...
    <div className="space-y-8">
      <section className="border-border bg-card rounded-3xl border p-5">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="text-foreground mt-1 text-lg font-semibold">{focusNode.label}</div>
          </div>
        </div>
      ...
      <section>
        ...
        <ul className="space-y-3">
          ...
          <li className="border-border bg-card flex items-center justify-between rounded-2xl border px-4 py-3">
            <span className="text-foreground text-sm">Clients</span>
            <span className="text-foreground text-sm font-semibold">{focusStats.clientCount}</span>
          </li>
          <li className="border-border bg-card flex items-center justify-between rounded-2xl border px-4 py-3">
            <span className="text-foreground text-sm">Visible Children</span>
            <span className="text-foreground text-sm font-semibold">{focusStats.budgetCount}</span>
          </li>
        </ul>
      </section>
    </div>
      ...
      >
        ...
        {viewMode === 'category' ? (
          <div className="pointer-events-none absolute inset-x-6 bottom-6 z-20 rounded-2xl border border-amber-400/20 bg-amber-400/10 px-4 py-3 text-sm text-amber-100 shadow-[0_0_40px_rgba(251,191,36,0.08)]">
            Category POV is still a placeholder. Client POV now starts from category bubbles and
            drills into clients, then budgets.
          </div>
        ) : null}
      </div>
    </ExplorerShell>
