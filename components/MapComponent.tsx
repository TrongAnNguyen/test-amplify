...
export default function MapComponent() {
  ...
  return (
    ...
    >
      <div
        className="bg-background relative h-full cursor-grab overflow-hidden active:cursor-grabbing"
        onWheel={handleWheel}
          ...
          >
            ...
            {layoutLinks.map((link) => {
              ...
              if (currentRoot.category === 'person') {
                isLinkDimmed =
                  (link.source.category === 'person' && link.source.id !== currentRoot.id) ||
            ...
            {layoutNodes.map((node) => {
              const isExpandable =
                viewMode === 'industry' ? node.category !== 'executive' : node.category !== 'person'
              ...
              if (currentRoot.category === 'person') {
                ...
                isDimmed =
                  node.category === 'person' && !edges.base.has(`${currentRoot.id}|${node.id}`)
              }
              ...
              return (
                <g
                  ...
                  className={`group cursor-pointer transition-all duration-500 ease-out ${isDimmed ? 'opacity-20 hover:opacity-100' : 'opacity-100'}`}
                  onClick={(e) => {
                    e.stopPropagation()
                    handleNodeClick(node)
                  }}
                ...
                >
                  ...
                  <text
                    ...
                    textAnchor={anchor}
                    fill="var(--foreground)"
                    className={`group-hover:text-foreground text-[11px] transition-colors select-none ${node.depth === 0 ? 'text-sm font-bold tracking-widest' : 'font-medium'}`}
                  >
                    {node.label}
                  </text>
                  ...
                  {node.subLabel && (
                    <text
                      ...
                      className={`group-hover:text-foreground text-[9px] transition-colors select-none ${node.depth === 0 ? 'tracking-wide' : ''}`}
                    >
                      {node.subLabel}
                    </text>
                  )}
